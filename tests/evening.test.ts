import { isEvening, isNight, isMorning, isAfternoon } from "../src/index";
...
test("evening", () => {
  ...
  expect(isEvening()).toEqual(true);
  expect(isEvening()).toEqual(!isNight() && !isMorning() && !isAfternoon());
  jest.setSystemTime(new Date(DATES.DATE_2000_01_01_00_00));
  expect(isEvening()).toEqual(false);
  expect(isEvening()).toEqual(!isNight() && !isMorning() && !isAfternoon());
